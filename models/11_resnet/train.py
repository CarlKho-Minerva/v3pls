...
def train_and_evaluate():
    print("--- Model 12: Transfer Learning (ResNet50) ---")
    ...
    print("Resizing spectrograms for ResNet50...")

    def preprocess_specs(specs):
        processed = []
        for spec in specs:
            # Resize to 96x96 (ResNet supports this with include_top=False)
            ...
            # ResNet50 preprocess (expects BGR, zero-centered)
            # preprocess_input handles the specific requirements for ResNet
            pre = preprocess_input(rgb.astype(np.float32))
            processed.append(pre)

        return np.array(processed)

    X_resnet = preprocess_specs(X_specs)
    ...
    # 5. Split Data
    X_train, X_test, y_train, y_test = utils.get_data_splits(X_resnet, y_enc)

    # 6. Train
    print("Loading ResNet50...")
    # include_top=False allows custom input shape (min 32x32)
    ...
    # 9. Generate Report
    report = f"""# Model 12: ResNet50 Results

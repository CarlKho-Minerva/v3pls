    ...
    try:
        while True:
            ...
            facing_direction = await handle_action(
                action_queue, keyboard, facing_direction, is_walking,
                last_action_time, state, pressed_keys
            )
            
            # THEN, process locomotion based on the (potentially new) direction
            is_walking, facing_direction = await handle_locomotion(
                locomotion_queue, keyboard, is_walking, facing_direction,
                state, pressed_keys
            )
...
async def handle_locomotion(locomotion_queue, keyboard, is_walking, facing_direction, state, pressed_keys):
...
async def handle_action(action_queue, keyboard, facing_direction, is_walking, last_action_time, state, pressed_keys):
...
async def dashboard(state, queues):
    ...
    while True:
        state.watch_connected = (
            time.time() - state.last_watch_data_time
        ) < 2.0
        ...
        print(
            f"Actor State: {Colors.GREEN}{state.current_actor_state}{Colors.RESET}"
        )
